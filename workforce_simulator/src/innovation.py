...
# (for cross-functional mix), and which innovation phases it supports. A
# deterministic, auditable table in the spirit of the routing suitability sheet.
...
def score(result, routing_by_task: dict, burden: dict) -> Tuple[float, Dict[str, float]]:
    ...
    ``routing_by_task`` is accepted for symmetry/future use; phase grouping is
    derived from the assignments' skills. ``burden`` is the option's
    reviewer-burden dict (review/rework/net AI saved).
    """
